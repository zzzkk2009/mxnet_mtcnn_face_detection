...
class MtcnnDetector(object):
    ...
    def __init__(self,
        ...
        for i in range(num_worker):
            ...
            # workner_net = mx.mod.Module.load(models[0], 1, context=ctx)  # TypeError: can't pickle module objects
    ...
    def slice_index(self, number):
        ...
        def chunks(l, n):
            ...
            for i in range(0, len(l), n):  # range(start, stop[, step])
                yield l[i:i + n]
    ...
    def detect_face(self, img):
        ...
        scales = []
        m = MIN_DET_SIZE / self.minsize  # MIN_DET_SIZE=12,  minsize=20
        minl *= m  # 实际最小检测图像尺寸
        factor_count = 0
        while minl > MIN_DET_SIZE:
            scales.append(m * self.factor ** factor_count)  # scales = [m, m * 0.709, m * 0.709**2, ...]
            minl *= self.factor  # 缩小检测尺寸(factor=0.709)
            factor_count += 1
        ...
        # example: if num_worker=2 and len(scales)=5
        #        then self.slice_index(5) -> sliced_index=[range(0, 2), range(2, 4), range(4, 5)]
        total_boxes = []
        for batch in sliced_index:
            # local_boxes = self.Pool.map( detect_first_stage_warpper, \
            #         izip(repeat(img), self.PNets[:len(batch)], [scales[i] for i in batch], repeat(self.threshold[0])) )
            # print('repeat(self.threshold[0])=', repeat(self.threshold[0]))
            local_boxes = self.Pool.map(detect_first_stage_warpper,
        ...
        total_boxes = np.vstack(total_boxes)  # 将多个序列按行拼接成一个矩阵
        ...
        total_boxes = self.convert_to_square(total_boxes)  # 转换成方框
        total_boxes[:, 0:4] = np.round(total_boxes[:, 0:4])  # round 四舍五入

        #############################################
        # second stage
        #############################################
        num_box = total_boxes.shape[0]  # 候选人脸窗口个数
    ...
    def find_tfrom_between_shapes(self, from_shape, to_shape):
        ...
        for i in range(from_shape_points.shape[0]):
            temp_dis = np.linalg.norm(from_shape_points[i] - mean_from)  # 正则化
